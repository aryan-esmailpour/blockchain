    ...
    user.save((err, doc) => {
      if (err) {
        if(err.name = 'MongoError' && err.code === 11000) {
          logger.error(`The save action on User Collection with document ${req.body.email} has some errors: ${err}`)
          err.statusCode = 400
          err.clientCode = 9
    ...
    if (err) {
      ...
      user.comparePassword(req.body.password, (err, isMatch) => {
        if (err) {
          ...
          user.save((err, doc) => {
            if (err) {
              logger.error(`The save action on User Collection with document ${req.session.email} has some errors: ${err}`)
              err.statusCode = 500
              err.clientCode = 14
              err.clientMessage = 'در فرآیند تغییر گذرواژه مشکلی پیش آمده است!'
              err.title = 'خطا در سرور'
